---
title: "setMultiple — Batch Update Multiple Store Keys"
sidebarTitle: "setMultiple"
description: "setMultiple updates multiple keys in one render cycle. Accepts a plain object, function, or Immer draft mutator, with an optional replaceStore flag."
---
...
## Error Case — Invalid Return Value

If `nextStateOrFn` is a function and it returns something that is not a plain object — such as `null`, an array, or a primitive — `setMultiple` logs an error and aborts without touching state:

```
[Store Error]: State update failed. Expected an object or a function returning an object.
Please check your setMultiple arguments.
```
